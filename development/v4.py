...
class PerfectedInformationBottleneck:
    ...
    ### BUGFIX: Fixed search_beta_values with proper multiprocessing and progress bar
    def search_beta_values(self, beta_values: np.ndarray, depth: int = 1) -> Dict[float, Tuple[float, float]]:
        ...
        def optimize_beta(beta):
            ...
            for run in range(n_runs):
                ...
                max_iterations = 2000 if proximity < critical_zone_width else 1500
            ...
            with self._progress_lock:
                ...
                # Only print every 5% to avoid excessive output
                if progress_pct % 5 == 0 or self._current_progress == self._total_progress:
        ...
        for i in range(0, len(beta_values), batch_size):
            ...
            # Create a ThreadPoolExecutor for this batch
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = [executor.submit(optimize_beta, beta) for beta in batch]
                
                # Collect results as they complete
                for future in as_completed(futures):
                    try:
                        beta, result = future.result()
                        results[beta] = result
                    except Exception as e:
