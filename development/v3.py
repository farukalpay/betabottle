...
class PerfectedInformationBottleneck:
  ...
  ### ENHANCEMENT: Improved search with adaptive runs near critical regions
  def search_beta_values(self, beta_values: np.ndarray, depth: int = 1) -> Dict[float, Tuple[float, float]]:
    ...
    for beta in tqdm_auto(beta_values, desc="Evaluating β values", leave=False):
      ...
      if proximity < critical_zone_width:
        n_runs = 5  # More runs very close to target
      elif proximity < critical_zone_width * 3:
        n_runs = 3  # Moderate number of runs near target
      else:
  ...
  def normalize_rows(self, matrix: np.ndarray) -> np.ndarray:
    ...
    # Ensure no zeros to avoid log domain issues
  ...
  def calculate_marginal_z(self, p_z_given_x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    # Calculate p(z) = ∑_x p(x)p(z|x) using log-domain for stability
    ...
    for i in range(self.cardinality_x):
      for k in range(self.cardinality_z):
        if p_z_given_x[i, k] > self.epsilon:
          ...
          pz_x = mp.mpf(float(p_z_given_x[i, k]))
          log_term = mp.mpf(float(log_p_z_given_x[i, k] - log_p_z[k]))
          mi_zx += px * pz_x * log_term
      ...
      for k in range(self.cardinality_z):
        for j in range(self.cardinality_y):
          if self.p_y_given_x[i, j] > self.epsilon:
            # Safe computation using log-domain
            log_term = self.log_p_y_given_x[i, j] - log_p_y_given_z[k, j]
            kl_terms[k] += self.p_y_given_x[i, j] * log_term
  ...
  def staged_optimization(self, target_beta: float, 
    ...
    for stage, beta in enumerate(betas):
      ...
      if stage_proximity < 0.01:
        ...
        stage_max_iter = int(max_iterations * 1.5)  # More iterations
